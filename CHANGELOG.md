...
- **kiapi** / **kiapi-proxy**: Each server resolves a persistent relay `node_id` from its user data directory, injects it into the relay, and acquires a single-instance lock there so a second process cannot share the same node identity. `kiapi-proxy` gains a `kiapi_proxy.core.app` user-directory module and a `platformdirs` dependency.
