"""Z-Image LoRA finetune service entry (always-async worker-thread thunk)."""
...
def handle_train(ctx: AppContext, req: TrainRequest) -> tuple[JobResult, list[FileID]]:
    ...
    workdir = create_work_dir("train/zimage")
    try:
...
def _extract_dataset(zip_path: str, dest: Path) -> Path:
    ...
    raise CapabilityError(
        "dataset zip must contain images (.jpg/.jpeg/.png/.webp) at the top level "
        "or in a single subfolder"
    )
