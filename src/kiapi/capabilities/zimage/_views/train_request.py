"""Z-Image LoRA finetune request model. Always async (long-running).
...
class TrainRequest(BaseModel):
    ...
    model: str | None = Field(
        default=None,
        description=(
            "Model variant to finetune (see GET /v1/image/zimage/models). Omit for "
            "the default `turbo`."
        ),
    )
    ...
    steps: int | None = Field(
        ...
        description=(
            ...
            "default (turbo 9, base 20)."
        ),
    )
    guidance: float = Field(
        default=0.0,
        description="Guidance scale used during training.",
    )
    ...
    max_resolution: int | None = Field(
        default=512,
        description=(
            "Images are resized so their long side is at most this many pixels."
        ),
    )
    low_ram: bool = Field(
        default=True,
    ...
    timestep_low: int | None = Field(
        default=None,
        description=(
            "Lower diffusion-timestep bound to sample. Omit for the variant default "
            "(turbo 4, base 0)."
        ),
    )
