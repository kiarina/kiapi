...
kiapi/
  core/
    app/       # AppContext、startup wiring、user directory
...
解決は共有パッケージ `kiarina-utils-app` に委譲され、明示設定、XDG environment
variable、`platformdirs` の順で解決します。
...
設定は user の `settings.yaml` の `kiapi.core.app` セクションで行います。設定した
path の `~` は現在の user に対して展開します。
