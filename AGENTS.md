...
各パッケージは独立してバージョン管理・PyPI 公開され、`packages/<package>/CHANGELOG.md` を個別に持ちます。リリースタグは `<package>-v<version>` 形式です。ルートの `pyproject.toml` は workspace 設定と共有の lint/test 設定のみを持ちます。
