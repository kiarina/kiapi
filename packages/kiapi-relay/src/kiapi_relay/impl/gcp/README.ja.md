...
`liveness/{node_id}` へ `heartbeat_interval_s` ごとに liveness heartbeat を
publish し、正常終了時に削除します。requester は `liveness` を read し、
...
- private で uniform-access な GCS bucket を作成し（default 名 `{project_id}-kiapi`、
  default region `asia-northeast1`）、`sessions/` object を 1 日で削除する
...
relay object は bucket と database の root 直下に置かれます。1 つの bucket や database を
複数環境で共有せず、relay デプロイごとに専用の bucket と RTDB instance を使用してください。
...
dir の `<data_dir>/node_id`、または `liveness` の一覧から取得してください）。
...
- 専用 private bucket と専用 RTDB instance を使用する
...
- GCS object name に secret を含めない
...
| `heartbeat_interval_s` | `KIAPI_RELAY_GCP_HEARTBEAT_INTERVAL_S` | `300.0` | kiapi node が `liveness/{node_id}` の liveness を更新する間隔。 |
