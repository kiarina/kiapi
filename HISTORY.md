...
- 検索結果ごとに「Use in Fetch」ボタンを追加。押すと Fetch タブへ切り替えて結果 URL を入力し、
  ページ取得は Fetch ボタンを押すまで実行しない。スマホ幅ではフォームへスクロールする
- 独立した kiapi で「岐阜」の検索結果から観光サイトの URL を引き継ぎ、ボタン押下直後に `web.fetch` ジョブが
  0 件のままと確認。Fetch 押下後は該当 URL の Markdown を取得した。検証で作ったファイルとジョブは削除した。
  ライト・ダークとスマホ幅 375px（横スクロールなし）も確認した


## 2026-09-26 — Web search のカテゴリとエンジンを選択式にした

- Web UI の自由入力だった `categories` と `engines` を複数選択のプルダウンへ変更。稼働中の SearXNG の
  `/config` から候補を取得する API を加え、空欄なら従来どおりサーバーの既定値を使う
- `make` と 372 件の単体テストを通した。独立した kiapi で候補がカテゴリ 31 件・有効なエンジン 85 件と確認し、
  UI で `general` と `bing` を選んで「岐阜」10 件を取得。ライト・ダークとスマホ幅 375px で確認した
