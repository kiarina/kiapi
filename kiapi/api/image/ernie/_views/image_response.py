"""``/v1/image/ernie/{generate,edit}`` result payload (the Job ``result`` shape).

Mirrors the dict returned by ``capabilities/ernie/_models/ernie._store_image`` so
...
``api/image/ernie/_views``. ``generate`` and ``edit`` produce the same shape, so
...
class ImageResponse(BaseModel):
    """Capability-specific ``result`` for a succeeded ernie generate/edit job."""
