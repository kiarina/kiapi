"""``/v1/image/zimage/generate`` result payload (the Job ``result`` shape).

Mirrors the dict returned by ``capabilities/zimage/_models/zimage._store_image``
...
class ImageResponse(BaseModel):
    """Capability-specific ``result`` for a succeeded zimage generate job."""
    ...
    params: dict[str, Any] = Field(
        description=(
            ...
            "width, height, steps, guidance, quantize, format, quality, loras), so "
